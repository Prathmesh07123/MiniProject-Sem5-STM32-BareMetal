...
unsigned char command_ESP_3[50]="AT+CWJAP=\"IOTTEST\",\"pendrive1\"\r\n"; // WIFI ACC-
...
void Delay_ms(uint32_t ms)
{
    ...
    for(i = 0; i < ms; i++)           // outer loop = 1 ms
    {
        for(j = 0; j < 8000; j++)     // inner loop = ~1 ms at 72 MHz
        {
            // just waste cycles
        }
    }
